/**
 * Página da categoria Alfaiataria — /alfaiataria
 *
...
export const metadata: Metadata = {
  title: "Alfaiataria | Belezuura",
  description: "Peças de alfaiataria com estilo e elegância na Belezuura.",
};
    ...
    <article className="group">
      <Link
        ...
        <div className="relative aspect-3/4 bg-rose-soft rounded-2xl overflow-hidden mb-3">
          ...
          {!product.inStock && (
            <div className="absolute top-2.5 left-2.5">
              <span className="bg-plum-dark/80 text-rose-soft px-2 py-0.5 rounded-md text-[10px] font-semibold tracking-wide uppercase">
                Esgotado
              </span>
            </div>
          )}
        ...
        <div className="px-0.5 space-y-0.5">
          <h3 className="text-[13px] md:text-sm font-medium text-plum-dark leading-snug group-hover:text-sage transition-colors line-clamp-2">
            {product.name}
          </h3>
...
export default async function AlfaiatariaPage({
  searchParams,
}: {
  ...
  const { items: products, total } = await getProductsByCollection(
    "alfaiataria",
    { sortBy, limit: 60 },
  );
